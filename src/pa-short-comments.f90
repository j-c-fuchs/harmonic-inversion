...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  ! calculates the resonaces (the results are not transformed (form d^))
  public :: pa_all
  ! transform results to normal form (form d)
  public :: pa_skal
  ! transform results to fourier-like form (form d~)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine pa_all(N, K, tau, prec_deg, MEAN, GEOMETRIC,&
    ...
    ! the diagonalisation of the Hessenberg Matrix
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine pa_coeffa(N,K,c,a)
    ...
    ! for zgesv, zgesvx, zsysv, zsysvx
    integer, dimension(K) :: IPIV

!!$    ! for zgesvx
!!$    complex(dp), dimension(K,K) :: zgesvx_AF
!!$    integer,     dimension(K)   :: zgesvx_IPIV
!!$    character    :: zgesvx_EQUED
!!$    real(dp),    dimension(K)   :: zgesvx_R
!!$    real(dp),    dimension(K)   :: zgesvx_C
!!$    complex(dp), dimension(K)   :: zgesvx_B
!!$    real(dp)     :: zgesvx_RCOND
!!$    real(dp),    dimension(1)   :: zgesvx_FERR
!!$    real(dp),    dimension(1)   :: zgesvx_BERR
!!$    complex(dp), dimension(2*K) :: zgesvx_WORK
!!$    real(dp),    dimension(2*K) :: zgesvx_RWORK

!!$    ! for zsysv
!!$    integer :: zsysv_LWORK
!!$    complex(dp), dimension(K-1) :: zsysv_WORK

!!$    ! for zsysvx
!!$    complex(dp), dimension(K,K) :: zsysvx_AF
!!$    complex(dp), dimension(K)   :: zsysvx_B
!!$    real(dp) :: zsysvx_RCOND
!!$    real(dp), dimension(1) :: zsysvx_FERR
!!$    real(dp), dimension(1) :: zsysvx_BERR
!!$    complex(dp), dimension(4*K) :: zsysvx_WORK
!!$    integer :: zsysvx_LWORK
!!$    real(dp), dimension(K) :: zsysvx_RWORK

!!$    ! for zgerfs
!!$    real(dp),    dimension(1)   :: FERR
!!$    real(dp),    dimension(1)   :: BERR
!!$    complex(dp), dimension(2*K) :: WORK
!!$    real(dp),    dimension(K)   :: RWORK

!!$    ! for zgelss
!!$    real(dp),    dimension(K)    :: singular
!!$    real(dp)    :: zgelss_RCOND
!!$    integer     :: zgelss_RANK
!!$    complex(dp), dimension(10*K) :: zgelss_WORK
!!$    integer     :: zgelss_LWORK
!!$    real(dp),    dimension(5*K)  :: zgelss_RWORK
...
!!$    ! OR: routine with 'iterative refinement'
!!$    call zgesvx('N','N',K,1,M,K,zgesvx_AF,K,zgesvx_IPIV,zgesvx_EQUED,&
!!$         &zgesvx_R,zgesvx_C,zgesvx_B,K,a,K,zgesvx_RCOND,zgesvx_FERR,&
!!$         &zgesvx_BERR,zgesvx_WORK,zgesvx_RWORK,INFO)
!!$    ! treat errors
!!$    if (INFO /= 0) then
!!$       write(0,*) "pa_coeffa: error",INFO,"in function 'zgesvx'&
!!$            & to compute the Polynom P"
!!$       call exit(2)
!!$    end if

!!$    ! OR: routine for complex symmetric matrices
!!$    call zsysv('U',K,1,M,K,IPIV,a,K,zsysv_WORK,zsysv_LWORK,INFO)
!!$    ! treat errors
!!$    if (INFO /= 0) then
!!$       write(0,*) "pa_coeffa: error",INFO,"in function 'zsysv'&
!!$            & to compute the Polynom P"
!!$       call exit(2)
!!$    end if

!!$    ! OR: routine for complex symmetric matrices with 'iterative refinement'
!!$    call zsysvx('N','U',K,1,M,K,zsysvx_AF,K,IPIV,zsysvx_B,K,a,K,&
!!$         &zsysvx_RCOND,zsysvx_FERR,zsysvx_BERR,zsysvx_WORK,&
!!$         &zsysvx_LWORK,zsysvx_RWORK,INFO)
!!$    ! treat errors
!!$    if (INFO /= 0) then
!!$       write(0,*) "pa_coeffa: error",INFO,"in function 'zsysvx'&
!!$            & to compute the Polynom P"
!!$       call exit(2)
!!$    end if

!!$    ! TEST: use routine for under-determined LGS if INFO > 0
!!$    if (INFO > 0) then
!!$       write(0,*) "pa_coeffa: warning: the LGS is underdetermined"
!!$       do j=1, K
!!$          do i=1, K
!!$             M(i,j) = c(i+j)
!!$          end do
...
!!$       a(1:K) = c(1:K)
!!$       zgelss_RCOND = 1d-5
!!$       call zgelss(K,K,1,M,K,a,K,singular,zgelss_RCOND,zgelss_RANK,&
!!$            &zgelss_WORK,zgelss_LWORK,zgelss_RWORK,INFO)
!!$       !Behandlung der Fehler
!!$       if (INFO < 0) then
!!$          write(0,*) "pa_coeffa: error",INFO,"in function 'zgelss'&
!!$               & to compute the Polynom P"
!!$          call exit(2)
!!$       end if
!!$    end if
...
!!$       a(1:K) = c(1:K)
!!$       zgelss_RCOND = -1d0
!!$       call zgelsy(K,K,1,M,K,a,K,zgelsy_JPVT,singular,zgelss_RCOND,&
!!$            &zgelss_RANK,zgelss_WORK,zgelss_LWORK,zgelss_RWORK,INFO)
!!$       !Behandlung der Fehler
!!$       if (INFO < 0) then
!!$          write(0,*) "pa_coeffa: error",INFO,"in function 'zgelss'&
!!$               & to compute the Polynom P"
...
!!$    ! routine for iterative refinement
!!$    call zgerfs('N',K,1,M2,K,M,K,IPIV,a2,K,a,K,FERR,BERR,WORK,RWORK,&
!!$         &INFO)
!!$    ! treat error
!!$    if (INFO /= 0) then
!!$       write(0,*) "pa_coeffa: error",INFO,"in function 'zgerfs'&
!!$            & to compute the Polynom P"
!!$       call exit(2)
!!$    end if
  ...
  subroutine pa_paramz(N,K,a,z)
    ...
    ! to compute the eigenvalues of the Hessenberg matrix.
